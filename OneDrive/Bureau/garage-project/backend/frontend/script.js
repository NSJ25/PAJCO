const API = "http://172.20.10.2:5000";   // ton backend Flask
const PICO_ENTREE = "http://172.20.10.3:8081/ouvrir";
const PICO_SORTIE = "http://172.20.10.3:8081/sortie";

// Boutons
document.getElementById("btn-entree").onclick = function() { envoyer("entree"); }
document.getElementById("btn-sortie").onclick = function() { envoyer("sortie"); }
...
function envoyer(action){
    ...
    .then(data => {
        if(data.access){
            ...
            let url = action === "entree" ? PICO_ENTREE + "?badge=" + badge 
